...
import { Patio, Garage, Bathroom, Job, Currency, JobLevel, Schedule, ContractType } from './icons';

export default function EmpleoCard({ empleo }) {
  const { area, type, currency, salary, level, schedule, contractType, title, description, location, images, user, createdAt } = empleo;
        ...
        <div className="w-full border-r border-b border-l border-gray-400 lg:border-l-0 lg:border-t lg:border-gray-400 bg-white rounded-b lg:rounded-b-none lg:rounded-r p-4 flex flex-col justify-between leading-normal">
          <div className="mb-8">
            <p className="flex justify-between text-sm text-gray-600 flex items-center pb-2">
              {location.street} {location.streetnumber}, {location.village}, {location.city} <span>{formatDate(createdAt)}</span>
            </p>
            <div className="text-gray-900 font-bold text-xl mb-2"><span className="text-gray-500">{type}</span> · {title}</div>
