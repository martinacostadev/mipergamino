...
postRouter.get('/alquileres', (req, res, next) => {
  Post.find({ category: 'Alquileres' }, function (err, result) {
    if (err) {
      res.status(400).send({
        'success': false,
        'error': err.message
...
postRouter.get('/empleos', (req, res, next) => {
  Post.find({ category: 'Empleos' }, function (err, result) {
    if (err) {
      res.status(400).send({
        'success': false,
        'error': err.message
